...
function renderComboChart(subject, competitors) {
  ...
  // Find Max Reviews to scale the Left Axis (Round up to nearest 10)
  const maxReviews = Math.max(
    ...allHotels.map((h) => h.wtkSummary.review_count || 0),
  );
  const yMax = Math.ceil(maxReviews / 10) * 10 || 10; // Default to 10 if 0

  // 2. Generate Y-Axis Labels (Reviews)
  ...
  // Create 5 steps for the axis
  for (let i = 5; i >= 0; i--) {
    yAxisLeft += `<span>${Math.round((yMax / 5) * i)}</span>`;
  }
  yAxisLeft += `</div>`;

  // 3. Generate Bars & Line Points
  ...
  allHotels.forEach((h, index) => {
    ...
    // Bar Height (Relative to yMax)
    // 200px is the visual height of the chart area minus padding
    const barHeightPx = (reviews / yMax) * 200;

    // Bar Color (Use different color for Subject)
    const bg = index === 0 ? "#1F3B32" : "#3b556e"; // Green for subject, Blue for others
    ...
    // Line Chart Math
    // X: Center of the column. (Index + 0.5) / Count * Width
    // We assume SVG width is 500px (viewBox width)
    const x = ((index + 0.5) / barCount) * 500;

    // Y: Score 0-10 mapped to height 230px (top padding 20px)
    // Score 10 = Top (y=20), Score 0 = Bottom (y=250)
    const y = 250 - (score / 10) * 230;

    polylinePoints += `${x},${y} `;
  });

  // 4. Generate Chart Area (Bars + SVG)
  const chartArea = `
    <div style="grid-row: 1 / 2; grid-column: 2 / 3; position: relative; height: 250px;">
      ...
      <svg style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 2; overflow: visible;" viewBox="0 0 500 250" preserveAspectRatio="none">
        ...
        ${polylinePoints
          ...
          .map(
            (p) =>
              `<circle cx="${p.split(",")[0]}" cy="${p.split(",")[1]}" r="4" fill="#4472C4" stroke="#fff" stroke-width="2" />`,
          )
          .join("")}
      </svg>
    </div>`;

  // 5. Right Axis (Score 10-0)
  const yAxisRight = `
  ...
  // 8. Update Legend below (Optional clean up)
  // We remove the old complex legend since we don't have detailed sentiment for competitors
  container.innerHTML += `
